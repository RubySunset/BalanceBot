...
char obstacleDetected() {
  ...
  //     return 'b';
  // }
  else if (lightL > detectionThreshold) {
    return 'l';
  } else if (lightR > detectionThreshold) {
    return 'r';
  } else {
    return 'n';
  }
...
void ControlLoop(void* pvParameters) {
  while (true) {
    if (sample_flag == true) {
      ...
      compl_theta = 0.9987 * (compl_theta - gx * 0.0013 / 131) + 0.0013 * atan(-ay / sqrt(pow(ax, 2.0) + pow(az, 2.0))) * RAD_TO_DEG; 
...
//      Serial.println(compl_theta-16);
      ...
      //No stabilising wheels and frame
        ...
        if (movement_mode == 'm') {
          theta_setpoint = simple_PID_calc(sample_interval, velocity_setpoint, sensed_speed, 1, 0.001, 0, 0);
        }

        //theta_dash control- always necessary, present in both of other modes too
        control_speed = simple_PID_calc(sample_interval, theta_setpoint, compl_theta-16.1, 0, 2.578, 0.0355, 0.098);//tune the compl_theta-16 value between 16.5-15.5, depending on the setup, but I think cascaded outer loop would also solve the slight drift; more I value makes the system less responsive so this might be better lower
        speed_right = control_speed;
        speed_left = control_speed;
