...
double calibrateMPU(){
  for(int i = 0; i < 1000; i++){
    ...
    compl_theta = 0.999*(compl_theta+gy*0.002/131)+0.001*atan2(ax,az)*RAD_TO_DEG;
    delay(2);
  }
...
void setup() {
  ...
  I2Cdev::writeBits(0x68, 0x1A, 2, 3, 0);
...
void loop() {
  ...
  if(sample_flag == true){
    ...
    int accx = ax/1638;
    int accz = az/1638;
    compl_theta = COMPL_ALPHA*(compl_theta + theta*0.002)-(1-COMPL_ALPHA)*atan2(accx,accz)*RAD_TO_DEG;

    if(k>filter_adjustment_interval){
      integral_theta = 90-compl_theta;
      k=0;
    }

    //Control Code
    double control_speed = simple_PID_calc(0.002, 0, /*90-compl_theta*/integral_theta, 50, 0, 0.5);//relevant variables go here
